...
bool Item::getVectorField(Sys::Sphere& sphere, Vector3f& vec)
{
	...
	Vector3f bridgeVec = mPosition;
	bridgeVec          = bridgeVec - getBridgeZVec() * 20.0f;
	Vector3f pos       = bridgeVec - sphere.mPosition;
	...
	if (dist >= 300.0f) {
		...
		if (absF(x) < 60.0f) {
			...
			if (z > -10.0f) {
				bridgeVec = getDirection(mFaceDir);
				bridgeVec *= -1.0f;
			} else {
				bridgeVec = getPerpDirection(mFaceDir);
				if (x > 0.0f) {
					bridgeVec *= -1.0f;
				}
			}
