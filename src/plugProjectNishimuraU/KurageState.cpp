...
void StateDead::init(EnemyBase* enemy, StateArg* stateArg)
{
	...
	kurage->m_velocity2 = Vector3f(0.0f);
...
void StateAttack::exec(EnemyBase* enemy)
{
	...
	if (kurage->m_animKeyEvent->m_running) {
		...
		} else if ((u32)kurage->m_animKeyEvent->m_type == 1) {
			if (kurage->isFinishMotion()) {
...
{
	Obj* kurage = static_cast<Obj*>(enemy);
	if (kurage->m_stickPikminCount == 0 || kurage->m_stateTimer > static_cast<Parms*>(kurage->m_parms)->m_properParms.m_fp10.m_value) {
		kurage->finishMotion();
	}
	...
	if (kurage->m_animKeyEvent->m_running && (u32)kurage->m_animKeyEvent->m_type == 1000) {
		if (kurage->m_health <= 0.0f) {
			transit(kurage, KURAGE_Dead, nullptr);
		} else if (kurage->m_stickPikminCount != 0) {
