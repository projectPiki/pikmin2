

/*
 * --INFO--
 * Address:	........
 * Size:	000084
 */
void J3DUReportVCD(J3DShape *)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	0000E4
 */
void J3DUCountMtxGroupNum(J3DModelData *)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	000098
 */
void J3DUCountUseMtxNum(J3DModelData *)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	80015DE8
 * Size:	000008
 */
void J3DShapeMtx::getUseMtxNum() const
{
/*
.loc_0x0:
  li        r3, 0x1
  blr
*/
}

/*
 * --INFO--
 * Address:	........
 * Size:	0000E4
 */
void J3DUCountEdge(J3DShape *)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	000310
 */
void createEdgeListFromStrip(void *, int, int, int, __J3DUEdge *, unsigned long)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	00029C
 */
void createEdgeListFromFan(void *, int, int, int, __J3DUEdge *, unsigned long)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	000174
 */
void J3DUCreateEdgeList(J3DShape *, __J3DUEdge *)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	0000DC
 */
void createTriangleListFromStrip(void *, int, int, int, __J3DUTriangle *, unsigned long)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	0001E8
 */
void createTriangleListFromFan(void *, int, int, int, __J3DUTriangle *, unsigned long)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	000358
 */
void J3DUCreateTriangleList(J3DShape *, __J3DUTriangle *)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	000024
 */
void drawWireLineSkin(unsigned short, unsigned char, unsigned short, unsigned char)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	000010
 */
void drawWireLine(unsigned short, unsigned short)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	00007C
 */
void drawWireTriangle(unsigned short, unsigned char, unsigned short, unsigned char, unsigned short, unsigned char, int)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	0002B8
 */
void J3DUDrawWireFromStrip(void *, int, int, int, unsigned short)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	00019C
 */
void J3DUDrawWireFromFan(void *, int, int, int, unsigned short)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	00035C
 */
void J3DUDrawWireShapeDL(J3DShapeDraw *, _GXVtxDescList *, _GXCompType, unsigned char)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	0000C4
 */
void J3DUDrawWireShape(J3DShapePacket *)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	0000DC
 */
void J3DUDrawWireShape(J3DShape *)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	000014
 */
void drawVertexSkin(unsigned short, unsigned char, unsigned long)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	000010
 */
void drawVertex(unsigned short, unsigned long)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	00002C
 */
void drawStripVertex(unsigned short, unsigned char, int, unsigned long)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	000280
 */
void J3DUDrawStripFromStrip(void *, int, int, int, unsigned short)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	000280
 */
void J3DUDrawStripFromFan(void *, int, int, int, unsigned short)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	0005FC
 */
void J3DUDrawStripShapeDL(J3DShapeDraw *, _GXVtxDescList *, _GXCompType, unsigned char)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	0000C8
 */
void J3DUDrawStripShape(J3DShapePacket *)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	0000E0
 */
void J3DUDrawStripShape(J3DShape *)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	00005C
 */
void vecToRGB(Vec *, _GXColor *)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	000158
 */
void drawVertexAttributeTriangle(unsigned short, Vec *, unsigned char, unsigned short, Vec *, unsigned char, unsigned short, Vec *, unsigned char, int)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	0004C4
 */
void J3DUDrawVertexAttributeFromStrip(void *, int, int, int, int, unsigned short)
{
	// UNUSED FUNCTION
}

/*
 * --INFO--
 * Address:	........
 * Size:	000200
 */
void J3DUDrawVertexAttributeShapeDL(J3DShapeDraw *, _GXVtxDescList *, _GXCompType, unsigned char)
{
	// UNUSED FUNCTION
}