...
void JStudio_JParticle::TAdaptor_particle::TJPACallback_::execute(JPABaseEmitter* emitter)
{
	...
	f64 fadeTimeEnd     = mAdaptor->_190;
	f64 fadeTimeCurrent = mAdaptor->_194;

	f64 alpha = 1.0;
	switch (mAdaptor->_18C) {
	case 1:
		alpha = fadeTimeCurrent / fadeTimeEnd;
		break;
	case 3:
		alpha = (fadeTimeEnd - fadeTimeCurrent) / fadeTimeEnd;
		break;
	}
	...
	if (!mAdaptor->_1A4) {
		...
		emitter->setGlobalTranslation(useSRT->getTranslation());
		...
		emitter->setGlobalScale(useSRT->getScaling());
	} else {
		Mtx mtx;
