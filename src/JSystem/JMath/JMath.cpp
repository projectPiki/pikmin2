...
void JMAEulerToQuat(short x, short y, short z, Quaternion* quat)
{
	...
	// int iY = (u16)(y/2) >> 5;
	// int iZ = ((z/2) >> 2) & 0x3FF8;
	float sinX = JMath::sincosTable_.m_table[(u16)(x / 2) / 32].first;
	// int iZ = ((z/2) >> 2) % 2048;
	float sinY = JMath::sincosTable_.m_table[(u16)(y / 2) / 32].first;
	float sinZ = JMath::sincosTable_.m_table[((u16)(z / 2) >> 2) & 0x3FF8].first;
	float cosX = JMath::sincosTable_.m_table[(u16)(x / 2) / 32].second;
	float cosY = JMath::sincosTable_.m_table[(u16)(y / 2) / 32].second;
	float cosZ = JMath::sincosTable_.m_table[((u16)(z / 2) >> 2) & 0x3FF8].second;
	// int iX = ((u16)(x/2)) >> 5;
