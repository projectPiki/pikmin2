...
namespace Nest {
	...
	void Mgr::loadModelData(JKRArchive* archive)
	{
		...
		enemyBMD
		    = JKRFileLoader::getGlbResource("enemy.bmd", loadNode->m_archive);
		if (enemyBMD != nullptr) {
	...
	void Mgr::doSimpleDraw(Viewport* viewport)
	{
		...
		for (int i = 0; i < getMaxObjects(); i++) {
			...
			PSMTXConcat(viewport->getMatrix(true)->m_matrix.mtxView, mtx1,
			            mtx2);
			...
			J3DMaterial* material
			    = modelData->m_jointTree.m_joints[0]->m_material;
	...
	void Mgr::initObjects()
	{
		...
		if (isValidEnemyTypeID()) {
			char* collisionFolder
			    = EnemyInfoFunc::getEnemyInfo(getEnemyTypeID(), 0xFFFF)
			          ->m_collName;
