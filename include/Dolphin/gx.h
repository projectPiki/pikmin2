...
//Name of the vertex attribute or array. Attributes are listed in the ascending order vertex data is required to be sent to the GP.
...
/*
* RGB, RGBA, Intensity, Intensity/Alpha, Compressed, and Z texture format types.
* See GXCITexFmt for information on color index formats.
* The CTF format is used only by the GXSetTexCopyDst function to specify how data is copied out of the EFB into a texture in main memory. 
* In order to actually use that texture, you must specify a non-copy format of matching size.
* For example, if copying using GX_CTF_RG8, you would apply the resulting texture using GX_TF_IA8.
*/
//Havent confirmed the GX docs match pikmin 2 here
...
typedef enum GXCompCnt {
	...
	GX_NRM_NBT3,	//Normal, binormal, tangent (three components). Use when NBT normal is indexed independently.
...
typedef enum _SDK_GXFogType {
	...
	GX_FOG_LINEAR     = 2,			//Use linear fog density function. For perspective projection mode.
	GX_FOG_EXPONENT   = 4,			//Use the exponential fog density function. For perspective projection mode.
	GX_FOG_EXPONENT2  = 5,			//Use the exponential-squared fog density function. For perspective projection mode.
	GX_FOG_REVERSEEXP = 6,			//Use the inverse exponential fog density function. For perspective projection mode.
	GX_FOG_REVERSEXP2 = 7,			//Use the inverse exponential-squared fog density function. For perspective projection mode.
...
* Each table GX_TLUT0 through GX_TLUT15 contains 256 entries, at 16 bits per entry.
* Each table GX_BIGTLUT0 through BIGTLUT3 contains 1024 entries, at 16 bits per entry.
* Used for setting texture memory in the GXInit function.
*/
typedef enum GXTlut {
...
typedef enum GXTlutFmt {
	...
	GX_TL_RGB5A3	//When MSB=1, RGB555 format (opaque), and when MSB=0, RGBA4443 format (transparent).
} GXTlutFmt;
