...
struct EnemyParmsBase : public CreatureParms {
	struct Parms : public Parameters {
		Parms()
		    ...
		    , m_lifeBeforeAlert(this, 'fp30', "警戒ライフ", 30.0f, 0.0f, 99999.0f)         // 'vigilant life'
		    , m_heightOffsetFromFloor(this, 'fp01', "マップとの当り", 40.0f, 0.0f, 100.0f) // 'match with the map'
		    , m_cellRadius(this, 'fp33', "マップとのあたりポリゴンの選定", 40.0f, 0.0f,
		    ...
		    , m_mass(this, 'fp05', "質量", 1.0f, 0.0f, 100.0f)                              // mass
		    ...
		    , m_fov(this, 'fp25', "視界高", 50.0f, 0.0f, 1000.0f)                           // visibility height
		    , m_viewAngle(this, 'fp13', "視界角度", 90.0f, 0.0f, 180.0f)                    // view angle
		    , m_searchDistance(this, 'fp14', "探索距離", 200.0f, 0.0f, 1000.0f)             // search distance
		    , m_searchHeight(this, 'fp26', "探索高", 50.0f, 0.0f, 1000.0f)                  // search height
		    , m_searchAngle(this, 'fp15', "探索角度", 120.0f, 0.0f, 180.0f)                 // search angle
		    ...
		    , m_maxAttackRange(this, 'fp20', "攻撃可能\範囲", 70.0f, 0.0f, 1000.0f)         // attack range - SHIFT-JIS IS WRONG
		    , m_minAttackRange(this, 'fp21', "攻撃可能\角度", 15.0f, 0.0f,
		                       180.0f)                                              // 'possible attack angle' - SHIFT-JIS IS WRONG
		    , m_attackRadius(this, 'fp22', "攻撃ヒット範囲", 70.0f, 0.0f, 1000.0f)  // attack hit range
		    , m_attackHitAngle(this, 'fp23', "攻撃ヒット角度", 15.0f, 0.0f, 180.0f) // attack hit angle
		    , m_attackDamage(this, 'fp24', "攻撃力", 10.0f, 0.0f, 1000.0f)          // attack power
		    , m_alertDuration(this, 'fp29', "警戒時間", 15.0f, 0.0f, 99.0f)         // alert time
		    , m_bitterDuration(this, 'fp35', "石化時間", 1.0f, 0.0f, 60.0f)         // stone time
		    , m_purplePikiStunDamage(this, 'fp36', "ヒップドロップダメージ", 10.0f, 0.0f, 1000.0f) // hip drop damage
		    , m_purplePikiStunChance(this, 'fp37', "地震気絶確立", 0.05f, 0.0f, 1.0f)              // earthquake faint probability
		    , m_purplePikiStunDuration(this, 'fp38', "地震気絶時間", 10.0f, 0.0f, 60.0f)           // earthquake faint time
		...
		Parm<f32> m_lifeBeforeAlert;        // _164, 'vigilant life'
		Parm<f32> m_heightOffsetFromFloor;  // _18C, 'match with the map'
		...
		Parm<f32> m_fov;                    // _3E4, 'visibility height'
		Parm<f32> m_viewAngle;              // _40C
		Parm<f32> m_searchDistance;         // _434, 'search distance'
		Parm<f32> m_searchHeight;           // _45C, 'search height'
		Parm<f32> m_searchAngle;            // _484, 'search angle'
		Parm<f32> m_shakeKnockback;         // _4AC
		Parm<f32> m_shakeDamage;            // _4D4
		Parm<f32> m_shakeRange;             // _4FC
		Parm<f32> m_shakeRateMaybe;         // _524
		Parm<f32> m_maxAttackRange;         // _54C, 'attack range'
		Parm<f32> m_minAttackRange;         // _574, 'possibly attack angle'
		Parm<f32> m_attackRadius;           // _59C, 'attack hit range'
		Parm<f32> m_attackHitAngle;         // _5C4, 'attack hit angle'
		Parm<f32> m_attackDamage;           // _5EC
		Parm<f32> m_alertDuration;          // _614, 'alert time'
		Parm<f32> m_bitterDuration;         // _63C
		Parm<f32> m_purplePikiStunDamage;   // _664
		Parm<f32> m_purplePikiStunChance;   // _68C
		Parm<f32> m_purplePikiStunDuration; // _6B4
