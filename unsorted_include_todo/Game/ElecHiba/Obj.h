...
struct EnemyBase {
	...
	virtual void lifeRecover();                                     // _2C0 (inline)
...
struct Obj : public EnemyBase, public Creature, public CellObject, public PelletView {
	...
	Obj();
	void getChildObjPtr();
	void setElecHibaPosition(Game::ElecHiba::InitialParam*, float);
	void interactDenkiAttack(Vector3<float>&);
	void addDamageMyself(float);
	void damageIncrement(float);
	void setupLodParms();
	void updateEfxLod();
	void createEffect(bool);
	void startChargeEffect(Game::Creature*);
	...
	void addAttrAttackCount(Game::Piki*);
	void isWaitFinish();
	void isAttackFinish();
};
} // namespace ElecHiba
