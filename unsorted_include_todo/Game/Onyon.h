...
namespace Game {
struct Onyon : public BaseItem, public Creature, public CellObject {
	virtual void onInit(CreatureInitArg*);                      // _30
	virtual void onKill(CreatureKillArg*);                      // _34
	virtual void doDirectDraw(Graphics&);                       // _50
	virtual void getFaceDir();                                  // _64 (weak)
	...
	void startPropera();
	void stopPropera();
	void setSpotState(Game::Onyon::cSpotState);
	void forceClose();
	void startWaitMotion();
	void getFlagSetPos();
	void vsChargePikmin();
	void doEmit(Game::Creature*, bool);
	void getLegPart(int);
	...
	void efxPafuKira();
	void getStoreCount();
	void enterPiki(Game::Piki*);
	void exitPikis(int, int);
	void exitPiki();
	void insideAccessArea(Vector3<float>&);
	void getInEnd_UFO();
	void getInStart_UFO();
