#ifndef _GAME_KOGANE_OBJ_H
#define _GAME_KOGANE_OBJ_H
...
struct PelletView {
	...
	virtual void createItem();                                 // _2FC (inline)
	virtual void resetFartTimer();                             // _300 (inline)
	virtual void startBodyEffect();                            // _304 (inline)
	virtual void finishBodyEffect();                           // _308 (inline)
	virtual void createFartEffect();                           // _30C (inline)
	virtual void effectDrawOn();                               // _310 (inline)
	virtual void effectDrawOff();                              // _314 (inline)
	virtual void createPressSENormal();                        // _318 (inline)
	virtual void createPressSESpecial();                       // _31C (inline)
	virtual void _320() = 0;                                   // _320
	virtual void _324() = 0;                                   // _324
	virtual void viewGetBaseScale();                           // _328 (inline)
	virtual void _32C() = 0;                                   // _32C
	virtual void viewGetCollTreeJointIndex();                  // _330 (inline)
	virtual void viewGetCollTreeOffset();                      // _334 (inline)
	virtual void _338() = 0;                                   // _338
	virtual void _33C() = 0;                                   // _33C
	virtual void _340() = 0;                                   // _340
	virtual void _344() = 0;                                   // _344
	virtual void _348() = 0;                                   // _348
	virtual void viewEntryShape(Matrixf&, Vector3<float>&);    // _34C (inline)
};
} // namespace Game
...
struct Obj : public EnemyBase, public Creature, public CellObject, public PelletView {
	...
	Obj();
	void transitDamageState(float);
	void transitDisappear();
	void getBodyJointPos();
	void koganeScaleUp();
	void koganeScaleDown();
	void setTargetPosition(Vector3<float>*);
	void resetAppearTimer();
	void isAppear();
	void resetMoveTimer(float, float);
	void createTreasureItem();
	void createPellet(int, int);
	void createDoping(unsigned char, int);
};
} // namespace Kogane
} // namespace Game
