...
struct EnemyBase {
	...
	virtual void applyImpulse(Vector3<float>&, Vector3<float>&);    // _18C (inline)
	...
	virtual void pressCallBack(Creature*, float, CollPart*);        // _27C (inline)
	...
	virtual void eatWhitePikminCallBack(Creature*, float);          // _298 (inline)
...
struct Obj : public EnemyBase, public Creature, public CellObject, public PelletView {
	...
	void getTonguePosVel(Vector3<float>&, Vector3<float>&);
	void setNextGoal();
	void searchTarget();
	void isOutOfTerritory(float);
	...
	void footMtxCalc(float (*)[4], Vector3<float>*, float*);
	void resetFootPos();
	void fadeAllEffect();
